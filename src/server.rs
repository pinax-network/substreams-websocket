...
impl ClientRegistry {
    ...
    /// skip the decode/serialize hot-path when nobody is listening (and replay
    /// is off). Biased to over-report so we never skip a stream that has a live
    /// subscriber: in discover mode (`tables` empty) the emitted tables aren't
    /// known ahead of decode, so any network match counts; a wildcard
    /// subscription (`*@*`, `network@*`) likewise matches.
