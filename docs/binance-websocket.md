...
- **`?from_block=<n>` reconnect protocol.** Binance has no concept of block-indexed replay. We retain the last N blocks per stream as JSONL on disk and replay matching ones on connect. See [`replay.md`](replay.md).
- **`gap` lifecycle status.** Emitted when `from_block` falls below the retained window. Tells the client to backfill via Substreams gRPC directly.
