...
> **Why not the Polymarket package here?** The WebSocket server only accepts DatabaseChanges (`db_out`). Polymarket's `db_out` is itself a `map`, but it **transitively depends on three stores** — `store_markets`, `store_traders`, `store_global_stats` (initial block **57,000,000**) — through its `map_market_orderbooks` / `map_trader_accounts` / `map_global_orderbook_stats` inputs. To emit at the live edge Substreams must first build those stores' state, so a `db_out --start-block -1` start fans out **~50 parallel backprocessing jobs** and churns tens of thousands of blocks before delivering a single live block (0 blocks after 75 s in the benchmark). By contrast `map_all_order_fills` (used by Tests 1–2) has no store dependency in its graph and streams at head instantly — but it isn't DatabaseChanges. The `evm-transfers` `db_out` has no such accumulating-store dependency, streams at head immediately, and stresses the fan-out path harder, so it's the better — and feasible — choice for measuring *server* overhead. The cold-start behaviour is itself worth noting for anyone wiring a store-dependent package into the live server.
