...
`BlockUndoSignal` carries `last_valid_cursor`. We persist that cursor and broadcast an `undo` envelope (see [`skills/SKILL.md`](../skills/SKILL.md)) so subscribed clients can roll back their local state. We never replay the undone blocks ourselves — that is the client's job using the original block deliveries.
