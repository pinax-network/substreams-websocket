...
- `skills/SKILL.md` — The on-wire message reference (the canonical client contract) **and** an installable Agent Skill (it carries YAML frontmatter, which is also [OKF](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md)-compliant). Compiled into the binary via `include_str!` and served at `GET /SKILL.md`. This is the single source of truth — there is no `public/SKILL.md`. The frontmatter's `timestamp: __BUILD_STAMPED__` placeholder is replaced at serve time with the file's last git-commit date (captured by `build.rs` as `SKILL_MD_TIMESTAMP`) — leave the placeholder literal in the file.
...
`skills/SKILL.md` is the on-wire contract (session message, block envelope, undo envelope, command envelope, error shapes). It is served at `GET /SKILL.md` at runtime and is what clients read. It also carries Agent Skills + OKF YAML frontmatter (`name`, `type`, `title`, `description`, `resource`, `tags`, `timestamp`, `license`) so it is an installable skill — keep the frontmatter valid when editing, and leave `timestamp: __BUILD_STAMPED__` as a literal placeholder (it is stamped at serve time). Any change to broadcast or response JSON requires updating this file — and probably `docs/binance-websocket.md` if you change URL or command conventions.
